    ...
    private boolean isAccountLock(String userName, UserStoreManager userStoreManager) {
        ...
        try {
            Map<String, String> values = userStoreManager
                    .getUserClaimValues(userName, new String[] { APIConstants.ACCOUNT_LOCKED_CLAIM },
                            UserCoreConstants.DEFAULT_PROFILE);
            accountLockedClaim = values.get(APIConstants.ACCOUNT_LOCKED_CLAIM);
            return Boolean.parseBoolean(accountLockedClaim);
        } catch (UserStoreException e) {
            log.error("Error occurred while retrieving " + APIConstants.ACCOUNT_LOCKED_CLAIM + " claim value", e);
        }
        return false;
    }
}
