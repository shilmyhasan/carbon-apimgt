...
public class MutualSSLAuthenticator implements Authenticator {
     ...
     *
     * @param messageContext        Relevant message context.
     * @param x509Certificates      SSL certificate chain.
     * @throws APISecurityException API Security exception.
     */
    private void setAuthContext(MessageContext messageContext, X509Certificate[] x509Certificates)
            throws APISecurityException, APIManagementException {

        String tier = null;
        String subjectDN = x509Certificates[0].getSubjectDN().getName();
        String issuerDNIdentifier = x509Certificates[x509Certificates.length - 1].getIssuerDN().getName().replaceAll(",", "#").replaceAll("\"", "'").trim();
        String uniqueIdentifier = null;
        List<String> subjectDNIdentifiers = new ArrayList<>();
        for (X509Certificate x509Certificate : x509Certificates) {
            String subjectDNIdentifier = (x509Certificate.getSerialNumber() + "_" + x509Certificate.getSubjectDN()).replaceAll(",",
                    "#").replaceAll("\"", "'").trim();
     ...
     */
    private String getTierFromCompleteCertificateChain(X509Certificate[] x509Certificates,
            List<String> uniqueIdentifiers) throws APIManagementException {

        String tier = null;
        X509Certificate certificate = x509Certificates[x509Certificates.length - 1];
        ...
        while (isIssuerCertificateUpdated) {
            java.security.cert.X509Certificate issuerCertificate = Utils.getCertificateFromListenerTrustStore(issuerDN);
