...
public class APIGatewayManager {
        ...
        for (String environmentName : api.getEnvironments()) {
			...
			if (client.getApi(tenantDomain, api.getId()) != null) {
				...
				if ((APIConstants.GATEWAY_ENV_TYPE_PRODUCTION.equals(environment.getType()) && !APIUtil.isProductionEndpointsExists(api)) ||
					...
					undeployCustomSequences(client, api,tenantDomain);
				} else {
					if (debugEnabled) {
                    ...
                    deployAPIFaultSequence(client, api, tenantDomain);
					...
					updateCustomSequences(client, api, tenantDomain);
				}
			} else {
				...
				if ((APIConstants.GATEWAY_ENV_TYPE_PRODUCTION.equals(environment.getType()) && !APIUtil.isProductionEndpointsExists(api)) ||
                    ...
                    if(!APIConstants.APIType.WS.toString().equals(api.getType())) {
                        ...
                        deployCustomSequences(client, api, tenantDomain);
                    } else {
	 ...
	 * 
	 * @param client APIGatewayAdminClient
	 * @param api -API object
	 * @param tenantDomain
	 */
    private void deployCustomSequences(APIGatewayAdminClient client, API api, String tenantDomain)
            throws APIManagementException {
        ...
        if (APIUtil.isSequenceDefined(api.getInSequence()) || APIUtil.isSequenceDefined(api.getOutSequence())) {
            try {
                ...
                if (APIUtil.isSequenceDefined(api.getInSequence())) {
                    deployInSequence(client, api, tenantId, tenantDomain);
                }

                if (APIUtil.isSequenceDefined(api.getOutSequence())) {
                	deployOutSequence(client, api, tenantId, tenantDomain);
                }
    ...
    private void deployInSequence(APIGatewayAdminClient sequenceAdminServiceClient, API api, int tenantId,
                                  String tenantDomain) throws APIManagementException, AxisFault {
	 ...
	 * @param tenantDomain
	 */
    private void undeployCustomSequences(APIGatewayAdminClient client, API api, String tenantDomain) {
	 ...
	 * @param tenantDomain
	 */
	private void updateCustomSequences(APIGatewayAdminClient client, API api, String tenantDomain)
            throws APIManagementException {
    ...
    private void deployAPIFaultSequence(APIGatewayAdminClient client, API api, String tenantDomain)
            throws APIManagementException {
     ...
     * @param securityAdminClient APIGatewayAdminClient
     * @param api
     * @param tenantDomain
     */
