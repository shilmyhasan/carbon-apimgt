...
public class CacheProvider {
     ...
     * @return cache
     */
    private static Cache getCache(final String cacheName, final long modifiedExp, long accessExp) {
    ...
    public static Cache createGatewayKeyCache() {
        ...
        if (apimGWCacheExpiry != null) {
            return getCache(APIConstants.GATEWAY_KEY_CACHE_NAME, Long.parseLong(apimGWCacheExpiry),
                    Long.parseLong(apimGWCacheExpiry));
        } else {
            long defaultCacheTimeout = getDefaultCacheTimeout();
            return getCache(APIConstants.GATEWAY_KEY_CACHE_NAME, defaultCacheTimeout, defaultCacheTimeout);
        }
    }
    ...
    public static Cache createResourceCache() {
        ...
        if (gatewayTokenCacheExpiry != null) {
            ...
            long defaultCacheTimeout = getDefaultCacheTimeout();
            return getCache(APIConstants.RESOURCE_CACHE_NAME, defaultCacheTimeout, defaultCacheTimeout);
        }
    }
    ...
    public static Cache createGatewayTokenCache() {
        ...
        if (apimGWCacheExpiry != null) {
            ...
            long defaultCacheTimeout = getDefaultCacheTimeout();
            return getCache(APIConstants.GATEWAY_TOKEN_CACHE_NAME, defaultCacheTimeout, defaultCacheTimeout);
        }
    ...
    public static Cache createInvalidTokenCache() {
        String apimGWCacheExpiry = getApiManagerConfiguration().getFirstProperty(APIConstants.TOKEN_CACHE_EXPIRY);
        if (apimGWCacheExpiry != null) {
            return getCache(APIConstants.GATEWAY_INVALID_TOKEN_CACHE_NAME, Long.parseLong(apimGWCacheExpiry),
                    Long.parseLong(apimGWCacheExpiry));
        } else {
            long defaultCacheTimeout = getDefaultCacheTimeout();
            return getCache(APIConstants
